...
func Astern(startLngInt, startLatInt, endLngInt, endLatInt,mWidth int64, meshgridPointer *[]bool) [][][]float64 {
		...
		if len(pq) == 0 {
			...
			for _, j := range neighbours {
				...
				if alt < dist[j] {
					...
					item := &Item {
						value: j,
						priority:-dist[j]- distance(gridToCoord(expandIndx(u)), gridToCoord([]int64{endLngInt,endLatInt})),
					}
					heap.Push(&pq,item)
...
func AsternAllNodes(startLngInt, startLatInt, endLngInt, endLatInt,mWidth int64, meshgridPointer *[]bool) ([][][]float64,[][]float64) {
	meshgrid = *meshgridPointer
	meshWidth = mWidth
	var dist []float64	
	var prev []int64
	...
	for {
		if len(pq) == 0 {
			...
			for _, j := range neighbours {
				var newDist = dist[u] + distance(gridToCoord(expandIndx(u)), gridToCoord(expandIndx(j)))
				if newDist < dist[j] {
					dist[j] = newDist
					prev[j] = u
					vertices[j] = true
