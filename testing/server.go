...
func neigbourghs1d(point int64) []int64 {
	...
	neigbourghs = append(neigbourghs, point+1)
	neigbourghs = append(neigbourghs, point-1)
	neigbourghs = append(neigbourghs, point-len(meshgrid2d))
	neigbourghs = append(neigbourghs, []int64{xMinus1, point[1]})
	neigbourghs = append(neigbourghs, []int64{xMinus1, yPlus1})
	neigbourghs = append(neigbourghs, []int64{xMinus1, yMinus1})
	neigbourghs = append(neigbourghs, []int64{point[0], yPlus1})
...
func min(dist *[]float64, vert *[]int64) []int64{
	var min = math.Inf(-1)
	var argmin int64
	var arschmin int64
	for j, i := range *vert {
	...
	return []int64{argmin, arschmin}
}

func dijkstra(startLng float64, startLat float64, endLng float64, endLat float64, startLngInt int64, startLatInt int64, endLngInt int64, endLatInt int64) [][]float64 {
	...
	print(len(dist))
	print("hello\n")
	// this is wrong i think, we need the length of meshgrid2d[0] glaub
	dist[(int64(len(meshgrid2d))*startLngInt)+startLatInt] = 0
